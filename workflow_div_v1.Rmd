...
Input files are: 
  ...
  + **MANDATORY FILE OTU or ASV table :**  It must be a .csv file containing the OTU or ASV table with OTU or ASV in column and sample names in rownames. This object must be a numeric matrix  
  ...
  + **MANDATORY FILE Taxonomic assignment table :** It must be a .csv file. This file must be composed of 2 columns: one called OTU_ID (name of the OTU or ASV) and one called user_supplied_name (taxonomic assignmenent of the OTU or ASV). A space between genus and species names is required (for now underscores or point are not accepted)
...
Identify the taxonomic assignments with no taxonomic classification found in WORMS and GBIF 
...
## Filter on non-target taxa

**Description:** Remove untargeted taxa if required by the user
...
If no user trait's table was imported or if some species from the community matrix where absent from the user traits' table (not done yet), 
go into Fishbase to retrieve traits 

**Remark:** It seems complicated for now to imagine the code in the case where we want to complement user's trait table if some species are absent because we need to have exact match between the name of the traits between user trait table and fishbase columns names- perhaps before at the begining of the module Traits, we could "force" the user to set very specific traits and use the name from Fishabse but it looks quite a bit messy --> Need to be discussed together
...
occ.species.target <- get.occ.species.obis(species.name = species.target, 
              startdate_chosen = NULL, # explain how to enter the data, format and look if its works
...
```{r}
otu.Genus <- taxo.table.class.target %>% filter(rank=="Genus") %>% distinct(valid_name)
otu.Genus      

occ.genus.obis(genus="Labrus",
               startdate_chosen = "2012", 
               type_zone="area", 
               coord_zone="40024")
```
